...
// @version      0.0.7
// @description  【全网最强】一键破除海角社区VIP限制，突破加密防盗链直接原画播放。内置高级HLS播放器(长按2X速/画中画)；无损下载(视频/语音)；强力拦截全端弹窗广告；支持自动展开帖子与本地自动登录，纯净看帖必备！
// @description:zh-CN 【全网最强】一键破除海角社区VIP限制，突破加密防盗链直接原画播放。内置高级HLS播放器(长按2X速/画中画)；无损下载(视频/语音)；强力拦截全端弹窗广告；支持自动展开帖子与本地自动登录，纯净看帖必备！
// @author       KEJIYU
// @license      MIT
// @match        *://*.haijiao.com/*
// @match        *://*/post/details*
...
const startPopupKiller = () => {
    const kill = () => {
        ...
        if (document.querySelector('.login-box, .login-form')) return;
 
        let isLogout = false;
        document.querySelectorAll('.el-message-box:not(.el-message-box--center)').forEach(mb => {
            if (mb.offsetHeight > 0) isLogout = true;
        });
        if (isLogout) return;
 
        let forceRemoved = false;
 
        document.querySelectorAll('.luodiconfirm, .van-dialog[class*="luodi"], div[role="dialog"][class*="luodi"], .luodi_ye_box').forEach(el => {
            const wrapper = el.closest('.van-overlay') || el.closest('.el-dialog__wrapper') || el.closest('.el-overlay');
            if (wrapper) wrapper.remove();
            else el.remove();
            forceRemoved = true;
        });
 
        document.querySelectorAll('.el-message-box--center').forEach(box => {
            const wrapper = box.closest('.el-message-box__wrapper');
            if (wrapper) wrapper.remove();
            else box.remove();
            forceRemoved = true;
        });
        ...
        if (timeCount) {
            const wrapper = timeCount.closest('.el-dialog__wrapper') || timeCount.closest('.el-overlay') || timeCount.closest('div[style*="z-index"]');
            if (wrapper) wrapper.remove();
            else timeCount.remove();
            forceRemoved = true;
        }
 
        document.querySelectorAll('.el-dialog__wrapper, .el-overlay').forEach(wrapper => {
            wrapper.querySelectorAll('p').forEach(p => {
                const text = p.textContent || '';
                if (text.includes('18周岁') || text.includes('自动复制')) {
                    wrapper.remove();
                    forceRemoved = true;
                }
            });
        ...
        document.querySelectorAll('.el-dialog__wrapper, .el-message-box__wrapper, .el-overlay, .van-dialog').forEach(el => {
            if (el.style.display !== 'none' && el.offsetHeight > 0 && document.body.contains(el)) {
        ...
        document.querySelectorAll('.van-overlay').forEach(overlay => {
            ...
            if (zIndex >= 2000 || forceRemoved || !hasVisibleDialog) {
                overlay.remove();
                forceRemoved = true;
            }
        });
 
        if (forceRemoved || (vModals.length > 0 && !hasVisibleDialog)) {
            vModals.forEach(m => m.remove());
        }
 
        if (forceRemoved || document.body.classList.contains('van-overflow-hidden') || document.body.style.overflow === 'hidden') {
...
history.pushState = function() { rs.apply(this, arguments); setTimeout(tickState, 500); };
const rr = history.replaceState;
history.replaceState = function() { rr.apply(this, arguments); setTimeout(tickState, 500); };
